  ...
  if ($(this).val() != "") {
    if (!$("#search .loader-wrapper").length) {
      ...
      $("#search .results")
        .show()
        .html(
          '<div class="loader-wrapper text-center"><div class="loader" style="width: 48px; height: 48px;"></div></div>'
        );
    }
    var query = $(this).val();
      ...
      xhr_search = $.ajax({
        ...
        beforeSend: function (jqXHR) {
          jqXHR.overrideMimeType(
            "text/html;charset=" + $("html meta[charset]").attr("charset")
          );
        },
        ...
        success: function (json) {
            ...
            if (group.results.length) {
              $("#search .results").append(
                "<h4>" +
                  group.name +
                  "</h4>" +
                  '<ul class="list-group" data-group="' +
                  group.name +
                  '"></ul>'
              );
              $.each(group.results, function (i, result) {
                $(
                  '#search .results ul[data-group="' + group.name + '"]'
                ).append(
                  '<li class="result">' +
          ...
          if ($("#search .results").html() == "") {
            $("#search .results").html(
              '<p class="text-center no-results"><em>:(</em></p>'
            );
          }
        },
      });
...
$(".nav-tabs").each(function () {
  ...
  $(this).on("click", '[data-toggle="tab"]', function (e) {
    ...
    history.replaceState(
      {},
      "",
      location.toString().replace(/#.*$/, "") + $(this).attr("href")
    );
  });
...
$("body").on(
  "click",
  '.data-table *[data-toggle="checkbox-toggle"], .data-table .checkbox-toggle',
  function () {
    $(this)
      .closest(".data-table")
      .find("tbody td:first-child :checkbox")
      .each(function () {
...
$('.data-table input[type="checkbox"]').click(function (e) {
  ...
  if (e.shiftKey) {
    ...
    $chkboxes
      ...
      .prop("checked", lastTickedCheckbox.checked);
  }
...
+(function ($) {
  $.fn.categoryPicker = function (config) {
    this.each(function () {
      ...
      $(this)
        .find('.dropdown input[type="search"]')
        .on({
          focus: function (e) {
          ...
          input: function (e) {
            ...
            if ($(this).val() == "") {
              $.getJSON(self.config.link + "?parent_id=0", function (result) {
                $(dropdownMenu).html(
                  '<li class="list-item"><h3 style="margin-top: 0;">' +
                    result.name +
                    "</h3></li>"
                );
                ...
                $.each(result.subcategories, function (i, category) {
                  $(dropdownMenu).append(
                    '<li class="list-item" data-id="' +
                      ...
                      category.name +
                      "</a>" +
                      '  <button class="add btn btn-default btn-sm" type="button">' +
                      self.config.translations.add +
                      "</button>" +
                      "</li>"
                  );
                });
            ...
            self.xhr = $.ajax({
              ...
              success: function (result) {
                if (!result.subcategories.length) {
                  $(dropdownMenu).html(
                    '<li class="list-item text-center no-results"><em>:(</em></li>'
                  );
                  return;
                }
                ...
                $(dropdownMenu).html(
                  '<li class="list-item"><h3 style="margin-top: 0;">' +
                    self.config.translations.search_results +
                    "</h3></li>"
                );
        ...
        $(self)
          ...
          .append(
            '<li class="list-item" style="display: flex;">' +
              ...
              $(category).data("name") +
              "</div>" +
              '  <button class="remove btn btn-default btn-sm" type="button">' +
  ...
  $.fn.categoryPicker = function (config) {
    this.each(function () {
                ...
                $.each(result.subcategories, function (i, category) {
                  $(dropdownMenu).append(
                    '<li class="list-item" data-id="' +
                      ...
                      category.name +
                      "</a>" +
                      '  <div><button class="add btn btn-default btn-sm" type="button">' +
                      self.config.translations.add +
                      "</button></div>" +
                      "</li>"
                  );
      ...
      $(this).on("click", ".dropdown-menu .list-item button.add", function (e) {
        ...
        $(self)
          .find('input[name="' + self.config.inputName + '"]')
          .each(function () {
        ...
        $(self)
          ...
          .append(
            '<li class="list-item" style="display: flex; align-items: center;">' +
              ...
              $(category).data("id") +
              '" data-name="' +
              $(category).data("name").replace(/"/, "&quote;") +
              '">' +
              '  <div style="flex-grow: 1;">' +
              self.config.icons.folder +
              " " +
              $(category).data("name") +
              "</div>" +
              '  <div><button class="remove btn btn-default btn-sm" type="button">' +
              self.config.translations.remove +
              "</button></div>" +
              "</li>"
          );
