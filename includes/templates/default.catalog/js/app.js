...
if (typeof window._env != "undefined") {
  if (window._env.template.settings.sidebar_parallax_effect == true) {
    var column = $("#sidebar > *:first-child"),
      sidebar = $("#sidebar");
    var sidebar_max_offset =
      $(sidebar).outerHeight(true) - $(column).height() - 20; // 20 = failsafe
    ...
    $(window)
      .on("resize scroll", function (e) {
        if (sidebar_max_offset) {
          ...
          if (
            $(window).width() >= 768 &&
            $(column).outerHeight(true) < $(sidebar).height()
          ) {
...
$("body").on(
  "click",
  '.data-table *[data-toggle="checkbox-toggle"]',
  function () {
    $(this)
      .closest(".data-table")
      .find("tbody :checkbox")
...
$("form").on(
  "input",
  'input[type="password"][data-toggle="password-strength"]',
  function () {
    ...
    var meter = $(
      '<meter min="0" low="80" high="120" optimum="150" max="150" value="' +
        score +
        '"></meter>'
    ).css({
      position: "absolute",
...
window.updateCart = function (data) {
  $.ajax({
    ...
    beforeSend: function (jqXHR) {
      jqXHR.overrideMimeType(
        "text/html;charset=" + $("meta[charset]").attr("charset")
      );
    },
  ...
  if (++num_cart_updates < 60) {
    // Continue refreshing up to 60 cycles
...
$("body").on("submit", 'form[name="buy_now_form"]', function (e) {
  ...
  var form = $(this);
  $(this)
    .find('button[name="add_cart_product"]')
    .animate_from_to("#cart", {
