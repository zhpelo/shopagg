...
if (empty($_GET['collections']) || in_array('translations', $_GET['collections'])) {
    $sql_union[] = "select 'translation' as entity, frontend, backend, code, date_updated, html,
                   " . implode(", ", array_map(function ($language_code) {
        return "`text_" . database::input($language_code) . "`";
    }, $_GET['languages'])) . "
...
$union_select = function ($entity, $entity_table, $info_table, $id, $field) {
    return (
        ...
        coalesce(" . implode(', ', array_map(function ($language_code) use ($field) {
            return "if($language_code.$field regexp '<', 1, null)";
        }, $_GET['languages'])) . ", 0) as html,
        " . implode(', ', array_map(function ($language_code) use ($field) {
            return "`" . database::input($language_code) . "`.$field as `text_" . database::input($language_code) . "`";
        }, $_GET['languages'])) . "
      from " . DB_TABLE_PREFIX . "$entity_table e
      " . implode(PHP_EOL, array_map(function ($language_code) use ($info_table, $id) {
            return "left join " . DB_TABLE_PREFIX . "$info_table `" . database::input($language_code) . "` on (`" . database::input($language_code) . "`.$id = e.id and `" . database::input($language_code) . "`.language_code = '$language_code')";
        }, $_GET['languages']))
    );
};
...
$translations_query = database::query(
    ...
    " . (!empty($_GET['untranslated']) ? "and (" . implode(" or ", array_map(function ($language_code) {
        return "`text_$language_code` = ''";
    }, $_GET['languages'])) . ")" : "") . "
    " . (!empty($_GET['query']) ? "and (code like '%" . addcslashes(database::input($_GET['query']), '%_') . "%' or " . implode(' or ', array_map(function ($language_code) {
        return "`text_" . database::input($language_code) . "` like '%" . addcslashes(database::input($_GET['query']), '%_') . "%'";
    }, $_GET['languages'])) . ")" : "") . "
